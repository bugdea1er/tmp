...
TYPED_TEST(file, create_invalid_openmode) {
  ...
  EXPECT_THROW(basic_file<TypeParam>(std::ios::trunc | std::ios::app),
               std::invalid_argument);
}

/// Tests file default open mode
TYPED_TEST(file, openmode_default) {
  basic_file<TypeParam> tmpfile = basic_file<TypeParam>();
...
TYPED_TEST(file, openmode_append) {
  basic_file<TypeParam> tmpfile = basic_file<TypeParam>(std::ios::app);
  tmpfile << "Hello, World!" << std::flush;
  tmpfile.seekg(0);
  tmpfile << "Goodbye!" << std::flush;
